...
namespace PRN211_Assignment2
{
    public partial class MainGUI : Form
    {
        ...
        private void albumsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (panelAdmin.Visible) { AdminLoad(); return; }
        ...
        private void shoppingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (panelShopping.Visible) { buttonSearch.PerformClick(); return; }
        ...
        private void buttonSearch_Click(object sender, EventArgs e)
        {
            ...
            _searchResult = Program.MusicStoreContext?.Albums.Include(e => e.Artist).Where(e =>
            e.Title.ToLower().Contains(title) &&
            (genre != null ? e.GenreId == genre.GenreId : true)
            ).ToList();
        ...
        private void SetSearchResult()
        {
            ...
            for (int i = 0; i < GBS.Length; i++)
                SetAlbum(GBS[i], (_currentPage < 0) ? null : (_currentPage * 3 + i < _searchResult?.Count) ? _searchResult?[_currentPage * 3 + i] : null);
        ...
        {
            var genre = (Genre)comboBoxAdminSearch.SelectedItem;
            string title = textBoxAdminSearch.Text.ToLower();

            AdminLoad(e => (genre != null ? e.GenreId == genre.GenreId : true) && e.Title.ToLower().Contains(title));
        }
